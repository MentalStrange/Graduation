...
function AddRadiologyCenterAppointment({ isOpen, onClose, onAppointmentAdded }) {
  const { radiologyCenters, loading: centersLoading, error: centersError } = usePatientState();
          ...
          {centersLoading ? (
            ...
            <FormControl>
              ...
              <Select placeholder="Select radiology center" onChange={(e) => setSelectedCenter(e.target.value)}>
                {radiologyCenters.data && radiologyCenters.data.data && radiologyCenters.data.data.map((center) => (
